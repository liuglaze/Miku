...
public class EndPoint : MonoBehaviour
{
    ...
    private void SkipVideoPlayback()
    {
        ...
        // 如果有正在执行的结束游戏序列协程，强制跳过视频部分
        if (endingRoutine != null)
        {
            StopCoroutine(endingRoutine);  // 停止当前的协程
            endingRoutine = StartCoroutine(EndGameSequence(afterVideo: true)); // 重新开始跳过视频后的协程
        }
    }
